...
### ✏️ Log
Add a meal or symptom entry in under 60 seconds. Meal logging includes ingredients, portion size, meal source (homemade / restaurant / takeout / canteen / friend's / packaged), food preparation state (raw / cooked / fermented / packaged), protein, calories, and an optional photo. Every saved meal is automatically checked against the FODMAP database — flagged ingredients show their FODMAP category and expected onset time. Symptom entries capture type, severity, onset speed, and likely trigger. Fast-onset reactions (30–60 min, fructose/polyols) are distinguished from slow-onset ones (1.5–4 hours, fructans/GOS).

### 🔍 Foods
A comprehensive, searchable FODMAP food database based on Monash University and NHS guidelines, covering fruits, vegetables, grains, proteins, dairy, nuts, condiments, drinks and sweeteners. Foods are rated safe 🟢, portion-dependent 🟡, or avoid 🔴. The database is dynamic — if a "safe" food repeatedly appears before symptom entries, the app flags it and suggests moving it to a personal watch list 🔵. Users can manually reassign any food at any time. Includes a full smart substitution guide for common triggers (e.g. garlic → garlic-infused oil, soy sauce → tamari, wheat pasta → rice noodles).
...
- Tracks **non-food triggers** alongside diet — stress, sleep, workout type and intensity, travel distance, electrolytes (which often contain polyols)
- **Dynamic safe/trigger lists** — no food is permanently fixed; the app learns from your logs
- Built for **fitness-conscious users** — protein and macros are first-class fields, not afterthoughts
- **Meal context matters** — logging where food came from and how it was prepared adds nuance most FODMAP apps ignore
- **Onset timing awareness** — fast vs slow FODMAP reactions are tracked separately to help connect symptoms to the right meal
- **Three full phases** — not just elimination, but a complete protocol through to a sustainable, personalised way of eating
- **Insights that surface patterns** — the trigger heatmap and wellbeing correlations turn a log into actual understanding
...
*Designed and built by Emy Osorio — [github.com/notasdemy]

---
© 2026 Emy Osorio. All rights reserved. 
This project is not open source. Please do not reproduce or distribute 
without permission.
